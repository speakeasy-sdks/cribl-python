...
class CriblMetadata:
    ...
    def get(self, accept_header_override: Optional[GetAcceptEnum] = None) -> operations.GetCriblMetadataResponse:
        ...
        hook_ctx = HookContext(operation_id='getCriblMetadata', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        ...
        res = operations.GetCriblMetadataResponse(http_meta=components.HTTPMetadata(request=req, response=http_res))
        
        if http_res.status_code == 200:
            if utils.match_content_type(http_res.headers.get('Content-Type'), 'text/xml'):                

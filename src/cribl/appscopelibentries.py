...
class AppscopeLibEntries:
    ...
    def get(self) -> operations.GetAppscopeLibEntriesResponse:
        r"""Get a list of AppscopeLibEntry objects
        Get a list of AppscopeLibEntry objects
        """
        hook_ctx = HookContext(operation_id='getAppscopeLibEntries', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        ...
        res = operations.GetAppscopeLibEntriesResponse(status_code=http_res.status_code, content_type=content_type, raw_response=http_res)
        
        if http_res.status_code == 200:
            if utils.match_content_type(content_type, 'application/json'):
                out = utils.unmarshal_json(http_res.text, Optional[components.AppScopeLibEntries])
                res.app_scope_lib_entries = out
            else:

...
class OutputSplunkHecUrls:
    url: Optional[str] = dataclasses.field(default='http://localhost:8088/services/collector/event', metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('url'), 'exclude': lambda f: f is None }})
...
class OutputSplunkHec:
    ...
    TYPE: Final[str] = dataclasses.field(default='splunk_hec', metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('type') }})
    auth_type: Optional[OutputSplunkHecAuthenticationMethod] = dataclasses.field(default=OutputSplunkHecAuthenticationMethod.MANUAL, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('authType'), 'exclude': lambda f: f is None }})
    r"""Enter a token directly, or provide a secret referencing a token"""
    ...
    failed_request_logging_mode: Optional[OutputSplunkHecFailedRequestLoggingMode] = dataclasses.field(default=OutputSplunkHecFailedRequestLoggingMode.NONE, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('failedRequestLoggingMode'), 'exclude': lambda f: f is None }})
    r"""Determines which data should be logged when a request fails. Defaults to None.  All headers are redacted by default, except those listed under `Safe Headers`."""
    ...
    next_queue: Optional[str] = dataclasses.field(default='indexQueue', metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('nextQueue'), 'exclude': lambda f: f is None }})
    r"""In the Splunk app, define which Splunk processing queue to send the events after HEC processing."""
    on_backpressure: Optional[OutputSplunkHecBackpressureBehavior] = dataclasses.field(default=OutputSplunkHecBackpressureBehavior.BLOCK, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('onBackpressure'), 'exclude': lambda f: f is None }})
    r"""Whether to block, drop, or queue events when all receivers are exerting backpressure."""
    ...
    pq_compress: Optional[OutputSplunkHecCompression] = dataclasses.field(default=OutputSplunkHecCompression.NONE, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('pqCompress'), 'exclude': lambda f: f is None }})
    r"""Codec to use to compress the persisted data."""
    ...
    pq_on_backpressure: Optional[OutputSplunkHecQueueFullBehavior] = dataclasses.field(default=OutputSplunkHecQueueFullBehavior.BLOCK, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('pqOnBackpressure'), 'exclude': lambda f: f is None }})
    r"""Whether to block or drop events when the queue is exerting backpressure (full capacity or low disk). 'Block' is the same behavior as non-PQ blocking. 'Drop new data' throws away incoming data, while leaving the contents of the PQ unchanged."""
    ...
    tcp_routing: Optional[str] = dataclasses.field(default='nowhere', metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('tcpRouting'), 'exclude': lambda f: f is None }})

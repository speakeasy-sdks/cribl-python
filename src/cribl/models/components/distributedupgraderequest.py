...
class DistributedUpgradeRequest:
    package_urls: Optional[List[PackageUrls]] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('packageUrls'), 'exclude': lambda f: f is None }})
    r"""Provide your own URLs or local paths for platform-specific Cribl packages."""
    upgrade_mode: Optional[UpgradeMode] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('upgradeMode'), 'exclude': lambda f: f is None }})

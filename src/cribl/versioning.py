...
class Versioning:
    ...
    def get(self) -> operations.GetVersioningResponse:
        r"""Get info about versioning availability
        Get info about versioning availability
        """
        hook_ctx = HookContext(operation_id='getVersioning', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        base_url = utils.template_url(*self.sdk_configuration.get_server_details())
        
        url = base_url + '/version/info'
        headers = {}
        ...
        res = operations.GetVersioningResponse(status_code=http_res.status_code, content_type=content_type, raw_response=http_res)
        
        if http_res.status_code == 200:
            if utils.match_content_type(content_type, 'application/json'):
                out = utils.unmarshal_json(http_res.text, Optional[components.GitInfos])
                res.git_infos = out
            else:

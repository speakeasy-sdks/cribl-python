...
class InternalSystemMetrics:
    ...
    def post(self, request: Optional[components.MetricsAggOpts]) -> operations.PostInternalSystemMetricsResponse:
        r"""Aggregate raw internal system metrics
        Aggregate raw internal system metrics
        """
        hook_ctx = HookContext(operation_id='postInternalSystemMetrics', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        base_url = utils.template_url(*self.sdk_configuration.get_server_details())
        
        url = base_url + '/system/metrics/query'
        headers = {}
        req_content_type, data, form = utils.serialize_request_body(request, Optional[components.MetricsAggOpts], "request", False, True, 'json')
        ...
        res = operations.PostInternalSystemMetricsResponse(status_code=http_res.status_code, content_type=content_type, raw_response=http_res)

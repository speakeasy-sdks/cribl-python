...
class CurrentConfig:
    ...
    def push(self) -> operations.PushCurrentConfigResponse:
        r"""push the current configs to the remote repository.
        push the current configs to the remote repository.
        """
        hook_ctx = HookContext(operation_id='pushCurrentConfig', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        base_url = utils.template_url(*self.sdk_configuration.get_server_details())
        
        url = base_url + '/version/push'
        headers = {}
        ...
        res = operations.PushCurrentConfigResponse(status_code=http_res.status_code, content_type=content_type, raw_response=http_res)
        
        if http_res.status_code == 200:
            if utils.match_content_type(content_type, 'application/json'):
                out = utils.unmarshal_json(http_res.text, Optional[components.PushConfig])
                res.push_config = out
            else:

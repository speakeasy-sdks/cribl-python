...
class Certificates:
    ...
    def get(self) -> operations.GetCertificatesResponse:
        r"""Get a list of Certificate objects
        Get a list of Certificate objects
        """
        hook_ctx = HookContext(operation_id='getCertificates', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        ...
        res = operations.GetCertificatesResponse(http_meta=components.HTTPMetadata(request=req, response=http_res))
        
        if http_res.status_code == 200:
            if utils.match_content_type(http_res.headers.get('Content-Type') or '', 'application/json'):                
                out = utils.unmarshal_json(http_res.text, Optional[components.Certificates])
